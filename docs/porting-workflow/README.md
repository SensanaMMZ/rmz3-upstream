# RMZ decomp upstream porting workflow

Scripts in `scripts/` expect two environment variables:
`RMZ3_FORK` (path to the matching-fork checkout) and `RMZ3_WT`
(path to the upstream worktree being ported into). Result and
plan files are written next to the scripts.
