...
# 1. Drop the re-added macro by taking upstream's header wholesale.
git checkout upstream/dev -- include/ 2>/dev/null

# 2. Every file that used it.
files=$(grep -rl "OBJECT_HDR;" src --include=*.c 2>/dev/null | xargs grep -L "COLLISION_OBJECT_HDR;" 2>/dev/null)
